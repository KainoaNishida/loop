...
Nudge is a macOS-first personal conversation assistant. It watches user-authorized Apple Messages locally, detects commitments and follow-up opportunities, and presents simple suggestions in a menu bar inbox.

The product exists for people who receive important requests in Messages and do not want those requests to disappear into history. Nudge turns conversations into reminders, calendar drafts, reply nudges, and lightweight follow-up actions without becoming another heavyweight inbox.
...
Messaging and email apps are good at communication, but weak at cross-thread memory. Calendar and reminder apps are good at commitments, but require manual translation. Nudge fills that gap by watching approved sources locally and surfacing action candidates.
...
"Nudge quietly watches the conversations you approve and turns important follow-ups into a small list of things worth doing next."
...
The tradeoff is setup friction. Nudge should treat onboarding as a serious product surface, not a small permissions dialog. The user should always know which automations are active, missing, degraded, or revoked.

## Core Experience

1. The user installs a notarized Mac app.
2. Nudge walks the user through an onboarding checklist.
3. The user grants selected permissions, especially Full Disk Access for local Apple Messages ingestion.
4. Nudge runs from the menu bar and imports recent Messages data read-only.
5. When Nudge detects something useful, it creates a suggestion.
6. The user opens the menu bar inbox, reviews the evidence, and confirms, snoozes, dismisses, or completes the suggestion.
7. For calendar/reminder actions, Nudge creates the item only after confirmation.
...
- User remains in control: Nudge drafts actions, but the user confirms external writes.
- Degrade honestly: missing permissions should be visible and understandable.
- Be honest about source access: Apple Messages is read locally without modifying the source database, and Nudge should not imply an official Messages integration.
